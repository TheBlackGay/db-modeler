...
public class TableDesignServiceImpl implements TableDesignService {
    ...
    @Override
    @Transactional
    public TableDesign importTableDesign(MultipartFile file, UUID projectId) throws IOException {
        ...
        try {
            ...
            ObjectMapper mapper = new ObjectMapper();
            JsonNode tableData = mapper.readTree(content);
            
            // 验证必需字段
            if (!tableData.has("code") || !tableData.has("displayName")) {
                throw new ValidationException("导入文件格式不正确，缺少必要字段");
            }

            TableDesign tableDesign = new TableDesign();
            tableDesign.setProjectId(projectId);
            tableDesign.setCode(tableData.get("code").asText());
            tableDesign.setDisplayName(tableData.get("displayName").asText());
            tableDesign.setComment(tableData.has("comment") ? tableData.get("comment").asText() : null);
            
            // 设置类型，默认为 TABLE
            String typeStr = tableData.has("type") ? tableData.get("type").asText().toUpperCase() : "TABLE";
            try {
                tableDesign.setType(TableDesign.Type.valueOf(typeStr));
            } catch (IllegalArgumentException e) {
                throw new ValidationException("无效的表类型: " + typeStr);
            }
            
            // 设置域，默认为 BUSINESS
            String domainStr = tableData.has("domain") ? tableData.get("domain").asText().toUpperCase() : "BUSINESS";
            try {
                tableDesign.setDomain(TableDesign.Domain.valueOf(domainStr));
            } catch (IllegalArgumentException e) {
                throw new ValidationException("无效的域类型: " + domainStr);
            }
            
            if (tableData.has("columns")) {
                tableDesign.setColumns(tableData.get("columns").toString());
            }
            
            if (tableData.has("metadata")) {
                tableDesign.setMetadata(tableData.get("metadata").toString());
            }
            ...
            return createTableDesign(tableDesign);
        } catch (IOException e) {
            logger.error("Failed to parse import file", e);
            throw new ValidationException("导入文件解析失败: " + e.getMessage());
        } catch (Exception e) {
            logger.error("Failed to import table design", e);
            throw new ValidationException("导入表设计失败: " + e.getMessage());
        }
    }

    @Override
    public byte[] batchExportTableSql(List<UUID> tableIds) {
        if (tableIds == null || tableIds.isEmpty()) {
            throw new ValidationException("表ID列表不能为空");
        }

        try {
            StringBuilder sqlBuilder = new StringBuilder();
            sqlBuilder.append("-- Generated by DB Modeler\n");
            sqlBuilder.append("-- Generated at: ").append(LocalDateTime.now()).append("\n\n");

            for (UUID tableId : tableIds) {
                TableDesign tableDesign = getTableDesignById(tableId);
                if (tableDesign == null) {
                ...
                // 添加表注释
                sqlBuilder.append("-- Table: ").append(tableDesign.getCode()).append("\n");
                if (StringUtils.hasText(tableDesign.getComment())) {
                    sqlBuilder.append("-- ").append(tableDesign.getComment()).append("\n");
                }
                sqlBuilder.append("-- -----------------------------------------------------\n");

                // 生成 DDL
                String ddl = ddlGeneratorService.generateCreateTableDDL(tableDesign);
            ...
            logger.error("Failed to generate batch DDL", e);
            throw new RuntimeException("生成批量DDL失败: " + e.getMessage());
        }
    }
}
